<script lang="ts">
    ...
    const updateUserPermission = async (userId, applicationId, roleId, permit) => {
        let applicationRole;
        const applicationRoles = data.applicationRoles.filter((ar) => ar.application_id === applicationId && ar.role_id === roleId);
        ...
        let permission;
        const permissions = data.permissions.filter((p) => p.user_id === userId && p.application_role_id === applicationRole.application_role_id);
                    ...
                    <div class="row">
                        ...
                        {#each data.roles as _, roleIndex}
                            <div class={`col-md-${Math.floor(10 / data.roles.length)}`}>
                                ...
                                {:else}
                                    {#if hasPermission(user.user_id, data.applications[applicationIndex].application_id, data.roles[roleIndex].role_id)}
                                    <i class="bi bi-toggle-on" on:click={() => deny(user.user_id, data.applications[applicationIndex].application_id, data.roles[roleIndex].role_id)}></i>
                                    {:else}
                                    <i class="bi bi-toggle-off" on:click={() => permit(user.user_id, data.applications[applicationIndex].application_id, data.roles[roleIndex].role_id)}></i>
                                    {/if}
                                {/if}
                            </div>
                        {/each}
